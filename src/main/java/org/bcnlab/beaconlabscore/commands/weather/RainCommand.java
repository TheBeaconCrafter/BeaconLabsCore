...
public class RainCommand implements io.papermc.paper.command.brigadier.BasicCommand {
    ...
    public void execute(io.papermc.paper.command.brigadier.CommandSourceStack stack, String[] args) {
        ...
        if (args.length > 0) {
            sender.sendMessage(plugin.getPrefix(sender).append(MiniMessage.miniMessage().deserialize("<gray>Usage: /rain")));
            return;
        }
